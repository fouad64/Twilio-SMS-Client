...
function renderSmsResults(data) {
    ...
    data.forEach(row => {
        ...
        const datePart = sentDate.toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric',
            year: 'numeric'
        });

        const timePart = sentDate.toLocaleTimeString('en-US', {
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit'
        });
        tr.innerHTML = `
            ...
            <td class="td-actions">
                <button class="delete-btn" onclick="showDeleteModal(${row.smsId})">
...
{
    return fetch('../GetAllSMSsByCustomerIDServlet')
